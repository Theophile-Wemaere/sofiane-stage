...
testData = ImageFolder(root="validation/", transform=transforms.ToTensor())
...
with torch.no_grad():
    ...
    for (image, label) in testDataLoader:
        ...
        numpy_array = image.numpy()[0]  # remove the batch dimension
        origImage = cv2.cvtColor(numpy_array.transpose(1, 2, 0), cv2.COLOR_RGB2BGR)
